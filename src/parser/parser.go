...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefix(token.LOGICAL_NOT, p.parsePrefixExpression)
...
func (p *Parser) peekError(t token.TokenType) {
	msg := "expected next token to be " + t + ", got " + p.peekToken.Type + " instead"
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	if err != nil {
		msg := "could not parse " + string(p.curToken.Literal) + " as integer"
		p.errors = append(p.errors, string(msg))
		return nil
	}
...
func (p *Parser) parseStringLiteral() ast.Expression {
	...
	lit.Value = string(p.curToken.Literal)
	return lit
}
